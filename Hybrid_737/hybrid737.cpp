...
#define LED_BLINK 5 // Upaljena je kod
#define LED_MODE  4	// Ako je upaljena onda smo Transmitter, ako je ugasena onda smo Receiver
...
ISR(INT0_vect) {
	IsTransmitter ^= 0xFF;				// Prebaci nacin
	PORTA		  ^= (1 << LED_MODE);	// Switchaj LEDicu
...
inline void Transmit() {	
	PORTA |= (0 << LED_BLINK); 	// #TODO -> ovo mi se cini da nije potrebno jer virtualwire sam po sebi flasha kao blesav dvije vanjske ledice :)
...
inline void Setup() {
	// Postavi LED
	DDRA  = (1 << LED_MODE) | (1 << LED_BLINK);
	PORTA = (1 << LED_MODE) | (1 << LED_BLINK);
...
int main() {
	...
	while(1) {
		if (IsTransmitter != 0) { // Radimo kao Transmitter
			Display();
			Input();
		} else {				  // Radimo kao Receiver
			if (GoodMessage != 0) {
				Display();
			}
			Receive();
